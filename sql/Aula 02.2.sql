...
insert into Associado 
  ...
  values  
      (2, 
	   ...
	   BEGIN TRANSACTION
	   update cidade set nome = 'Sao Leopoldo' where IDCidade = 1;

	   commit

	   insert into Associado (IDAssociado, Nome, DataNascimento, Sexo)
	   values (5, 'Julio de Castilhos', convert(datetime, '14/12/1947', 103), 'M');

	   insert into Associado (IDAssociado, Nome, DataNascimento, Sexo)
	   values (4, 'Antonio Augusto Borges de Medeiros', convert(datetime, '19/03/1942', 103), 'M');

	   insert into Associado (IDAssociado, Nome, DataNascimento, Sexo)
	   values (3, 'Osvaldo Aranha', convert(datetime, '08/02/1958', 103), 'M');
	   ...
	   select IDCidade, Nome, UF from Cidade;
		...
		delete from Associado where IDAssociado=4
		delete from Associado where IDAssociado=5
